const conf = {
    ...
    widsom: [
        'Luxuries burn easy',
        'The maker brings the loop that is tomorrow',
        'I can only show, it is up to you to learn',
        'Don\'t decide who\'s weight to bear, instead just wait',
        'When filled with malice stop seeking more in the well, challenge the habit',
        'We seek freedom in responsibility',
        'People with rights tend to be more inclined to work and produce wealth than slaves',
        'There is no control to be had, just focus.',
        'Water flows through you, not around you',
    ],
};
...
function openWindow(type, title) {
    ...
    if (type === 'file') {
        ...
        if (['.png', '.jpg', '.jpeg'].includes(fileExtension)) {
            newWindow.style.backgroundImage = `url('./img/${title}')`;
...
function createSubmenu() {
    const submenu = document.getElementsByClassName('body-subtitle submenu');
    for (let i = 0; i < submenu.length; i++) {
        ...
        element.addEventListener('click', () => {
            const bodyCopy = element.parentElement.getElementsByClassName('body-copy');
            for (let i = 0; i < bodyCopy.length; i++) {
            // console.log(element.parentElement);
                bodyCopy[i].classList.toggle('on');
            }
        });
    }
}
